...
import cl from 'src/components/elements/student-card-bottom/StudentCardBottom.module.scss';

type Props = Required<Pick<IStudentCard, 'chatsLinks' | 'nextLessonData'>>;

const StudentCardBottom: FC<Props> = ({ chatsLinks: { telegram, whatsapp }, nextLessonData }) => (
  <>
    ...
    <div className={cl.nextLesson}>
      ...
      <span className={cl.nextLessonVisualiser}>
        {`${convertToDateString(nextLessonData)} в ${convertToTimeString(nextLessonData)}`}
      </span>
    </div>
  </>
);
