import { dateTransformer } from '@app/rest/dateTransformer';
...
export class HttpClient {
  ...
  private readonly applyPrerequestHooks = async (): Promise<void> => {
    try {
      const qwe = async (hook: PrerequestHook) => {
        await hook();
      };
      Array.from(this.prerequestHooks).forEach(qwe);
  ...
  withPrerequestHooks = (hooks: PrerequestHook[]) => {
    Array.from(this.prerequestHooks).forEach(hook => {
